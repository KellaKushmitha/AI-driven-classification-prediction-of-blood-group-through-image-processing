...
def is_blood_sample_image(image_path):
    """
    Validates if the uploaded image is a blood sample image
    (reddish blob on a light/white background).
    Returns True if valid blood sample, False otherwise.
    """
    img = cv2.imread(image_path)
    if img is None:
        return False
    ...
    # Red color range in HSV (two ranges because red wraps around 180)
    ...
    # Convert to grayscale and check bright pixel ratio
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    bright_pixels = cv2.countNonZero(cv2.inRange(gray, np.array([180]), np.array([255])))
...
def PredictAction(request):
    if request.method == 'POST':
        ...
        # --- Validate: only allow blood sample images ---
        if not is_blood_sample_image('BloodApp/static/test.jpg'):
            context = {
                'data': '<font size="3" color="red"><b>&#10060; Invalid Image!</b> Please upload a valid blood sample test image (reddish drop on white/light background). '
                        'Face photos, screenshots, or unrelated images are not accepted.</font>'
            }
